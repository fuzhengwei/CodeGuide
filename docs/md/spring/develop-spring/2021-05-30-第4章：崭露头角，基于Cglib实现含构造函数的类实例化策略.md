...
- 在此次的单元测试中，依然包括包括三个核心步骤；初始化 BeanFactory 工厂、注册 Bean、获取 Bean，此外与上一章节不同的是，在获取 Bean 对象时候，传递了一个参数名称为“小傅哥”的入参信息，这个信息的传递将会帮我们创建出含有 String 类型构造函数的 UserService 类，而不会再出现初始化报错的问题。
