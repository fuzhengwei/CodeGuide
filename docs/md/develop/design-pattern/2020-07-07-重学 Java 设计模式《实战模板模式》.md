...
title: 重学 Java 设计模式：实战模板模式「模拟爬虫各类电商商品，生成营销推广海报场景」
tagline: by 小傅哥
...
- 图片来自：[https://refactoringguru.cn/design-patterns/template-method](https://refactoringguru.cn/design-patterns/template-method)
...
- 通过上面的实现可以看到`模板模式`在定义统一结构也就是执行标准上非常方便，也就很好的控制了后续的实现者不用关心调用逻辑，按照统一方式执行。那么类的继承者只需要关心具体的业务逻辑实现即可。
